...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Download</h2>
          <input type=button value='Tambah Download' onclick=location.href='?inyong=download&act=tambahdownload'>
          <table>
    ...
    while ($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                ...
                <td>$r[nama_file]</td>
                <td>$tgl</td>
                <td><a href=?inyong=download&act=editdownload&id=$r[id_download]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a> | 
	                  <a href=$aksi?inyong=download&act=hapus&id=$r[id_download]><img src='icon/delete-kecil.png' border=0px width=15px height=16px title='delete'></a>
		        </tr>";
    $no++;
    }
    echo "</table>";
  ...
  case "tambahdownload":
    echo "<h2>Tambah Download</h2>
          <form method=POST action='$aksi?inyong=download&act=input' enctype='multipart/form-data'>
          <table>
  ...
  case "editdownload":
    ...
    echo "<h2>Edit Download</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?inyong=download&act=update>
