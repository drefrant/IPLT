...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Kategori</h2>
          ...
          onclick=\"window.location.href='?inyong=kategori&act=tambahkategori';\">
          <table>
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             <td><a href=?inyong=kategori&act=editkategori&id=$r[id_kategori]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a>
             </td></tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahkategori":
    echo "<h2>Tambah Kategori</h2>
          <form method=POST action='$aksi?inyong=kategori&act=input'>
          <table>
  ...
  case "editkategori":
    ...
    echo "<h2>Edit Kategori</h2>
          <form method=POST action=$aksi?inyong=kategori&act=update>
