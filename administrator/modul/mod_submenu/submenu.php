...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Sub Menu</h2>
          <input type=button value='Tambah Sub Menu' onclick=\"window.location.href='?inyong=submenu&act=tambahsubmenu';\">
          <table>
    ...
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td>$no</td>
                ...
                <td>$r[nama_menu]</td>
                <td>$r[link_sub]</td>
		            <td><a href=?inyong=submenu&act=editsubmenu&id=$r[id_sub]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a> | 
		                <a href=$aksi?inyong=submenu&act=hapus&id=$r[id_sub]><img src='icon/delete-kecil.png' border=0px width=15px height=16px title='delete'></a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahsubmenu":
    echo "<h2>Tambah Sub Menu</h2>
          <form method=POST action='$aksi?inyong=submenu&act=input'>
          <table>
  ...
  case "editsubmenu":
    ...
    echo "<h2>Edit Sub Menu</h2>
          <form method=POST action=$aksi?inyong=submenu&act=update>
