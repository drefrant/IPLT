...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Album</h2>
          ...
          onclick=\"window.location.href='?inyong=album&act=tambahalbum';\">
          <table>
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             <td>$r[jdl_album]</td>
             <td><a href=?inyong=album&act=editalbum&id=$r[id_album]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a>
             </td></tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahalbum":
    echo "<h2>Tambah Album</h2>
          <form method=POST action='$aksi?inyong=album&act=input' enctype='multipart/form-data'>
          <table>
  ...
  case "editalbum":
    ...
    echo "<h2>Edit Album</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?inyong=album&act=update>
