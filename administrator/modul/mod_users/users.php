...
switch($_GET[act]){
  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
      ...
      echo "<h2>User</h2>
          <input type=button value='Tambah User' onclick=\"window.location.href='?inyong=user&act=tambahuser';\">";
    }
    else{
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             <td><a href=?inyong=user&act=edituser&id=$r[id_session]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a></td></tr>";
      $no++;
    }
    echo "</table>";
    break;
  ...
  case "tambahuser":
    ...
    echo "<h2>Tambah User</h2>
          <form method=POST action='$aksi?inyong=user&act=input'>
          <table>
  ...
  case "edituser":
    ...
    echo "<h2>Edit User</h2>
          <form method=POST action=$aksi?inyong=user&act=update>
