...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Templates</h2>
          ...
          onclick=\"window.location.href='?inyong=templates&act=tambahtemplates';\">
          <table>
    ...
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td>$no</td>
                ...
                <td><a href=?inyong=templates&act=edittemplates&id=$r[id_templates]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a> | 
	                  <a href=$aksi?inyong=templates&act=aktifkan&id=$r[id_templates]><img src='icon/aktif-kecil.png' border=0px width=15px height=16px title='aktifkan'></a>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahtemplates":
    echo "<h2>Tambah Templates</h2>
          <form method=POST action='$aksi?inyong=templates&act=input'>
          <table>
  ...
  case "edittemplates":
    ...
    echo "<h2>Edit Templates</h2>
          <form method=POST action=$aksi?inyong=templates&act=update>
