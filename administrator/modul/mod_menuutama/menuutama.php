...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Menu Utama</h2>
          ...
          onclick=\"window.location.href='?inyong=menuutama&act=tambahmenuutama';\">
          <table>
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             <td><a href=?inyong=menuutama&act=editmenuutama&id=$r[id_main]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a>
             </td></tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahmenuutama":
    echo "<h2>Tambah Menu Utama</h2>
          <form method=POST action='$aksi?inyong=menuutama&act=input'>
          <table>
  ...
  case "editmenuutama":
    ...
    echo "<h2>Edit Menu Utama</h2>
          <form method=POST action=$aksi?inyong=menuutama&act=update>
