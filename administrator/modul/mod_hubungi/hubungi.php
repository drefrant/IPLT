...
switch($_GET[act]){
  ...
  default:
    ...
    while ($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td>$r[nama]</td>
                <td><a href=?inyong=hubungi&act=balasemail&id=$r[id_hubungi]>$r[email]</a></td>
                <td>$r[subjek]</td>
                <td>$tgl</a></td>
                <td><a href=$aksi?inyong=hubungi&act=hapus&id=$r[id_hubungi]><img src='icon/delete-kecil.png' border=0px width=15px height=16px title='delete'></a>
                </td></tr>";
    $no++;
    }
    echo "</table>";
  ...
  case "balasemail":
    ...
    echo "<h2>Reply Email</h2>
          <form method=POST action='?inyong=hubungi&act=kirimemail'>
          <table>
  ...
  case "kirimemail":
    mail($_POST[email],$_POST[subjek],$_POST[pesan],"From: admin@kabarinyong.com");
