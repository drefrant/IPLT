...
switch($_GET[act]){
  ...
  default:
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td>$r[judul]</td>
                <td>$tgl_posting</td>
		            <td><a href=?inyong=halamanstatis&act=edithalamanstatis&id=$r[id_halaman]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a> | 
		                <a href='$aksi?inyong=halamanstatis&act=hapus&id=$r[id_halaman]&namafile=$r[gambar]'><img src='icon/delete-kecil.png' border=0px width=15px height=16px title='delete'></a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "edithalamanstatis":
    ...
    echo "<h2>Edit Halaman Statis</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?inyong=halamanstatis&act=update>
