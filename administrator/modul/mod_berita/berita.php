...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Berita</h2>
          ...
          <input type=hidden name=inyong value=berita>
          <div id=paging>Masukkan Judul Berita : <input type=text name='kata'> <input type=submit value=Cari></div>
          </form>
          <input type=button value='Tambah Berita' onclick=\"window.location.href='?inyong=berita&act=tambahberita';\">";
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td>$r[judul]</td>
                <td>$tgl_posting</td>
		            <td><a href=?inyong=berita&act=editberita&id=$r[id_berita]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a> | 
		                <a href=\"$aksi?inyong=berita&act=hapus&id=$r[id_berita]&namafile=$r[gambar]\" onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\"><img src='icon/delete-kecil.png' border=0px width=15px height=16px title='delete'></a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td>$r[judul]</td>
                <td>$tgl_posting</td>
		            <td><a href=?inyong=berita&act=editberita&id=$r[id_berita]>Edit</a> | 
		                <a href='$aksi?inyong=berita&act=hapus&id=$r[id_berita]&namafile=$r[gambar]'>Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahberita":
    echo "<h2>Tambah Berita</h2>
          <form method=POST action='$aksi?inyong=berita&act=input' enctype='multipart/form-data'>
          <table>
