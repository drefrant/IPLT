...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Galeri Foto</h2>
          <input type=button value='Tambah Galeri Foto' onclick=\"window.location.href='?inyong=galerifoto&act=tambahgalerifoto';\">
          <table>
    ...
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td>$no</td>
                <td>$r[jdl_gallery]</td>
                <td>$r[jdl_album]</td>
		            <td><a href=?inyong=galerifoto&act=editgalerifoto&id=$r[id_gallery]><img src='icon/edit-kecil.png' border=0px width=15px height=16px title='edit'></a> | 
		                <a href='$aksi?inyong=galerifoto&act=hapus&id=$r[id_gallery]&namafile=$r[gbr_gallery]'><img src='icon/delete-kecil.png' border=0px width=15px height=16px title='delete'></a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahgalerifoto":
    echo "<h2>Tambah Galeri Foto</h2>
          <form method=POST action='$aksi?inyong=galerifoto&act=input' enctype='multipart/form-data'>
          <table>
  ...
  case "editgalerifoto":
    ...
    echo "<h2>Edit Galeri Foto</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?inyong=galerifoto&act=update>
